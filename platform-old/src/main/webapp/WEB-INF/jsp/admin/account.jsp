...
<html>
			...
			if (ids) {
				$.messager.confirm('警告','您确认要封号吗?',function(data) {
					if (data) {
						$.messager.progress({title:'请稍后',msg:'提交中...'});
						$.ajax({
							url : '${ctx}/admin/account/lock?id=0&ids='+ids,
							type : 'GET',
							timeout : 1000,
							error : function() {
			...
			if (ids) {
				$.messager.confirm('警告','您确认要解封吗?',function(data) {
					if (data) {
						$.messager.progress({title:'请稍后',msg:'提交中...'});
						$.ajax({
							url : '${ctx}/admin/account/unlock?id=0&ids='+ids,
							type : 'GET',
							timeout : 1000,
							error : function() {
	...
	<body>
		...
		<form:form modelAttribute="account"
			action="${ctx}/admin/account/query"
			...
			<table>
				...
				<tr>
					...
					<td>
						<form:input path="gender" cssStyle="width:156px;"/>
					</td>
					<td>
		...
		<div style="text-align: center; padding: 5px;">
			...
			idField="id" url="${ctx}/admin/account/query" pagination="true" rownumbers="true"
		...
		<div id="editWin" class="easyui-window" title="账号" closed="true" style="width:700px;height:380px;padding:5px;" modal="true">
			<form:form modelAttribute="account" id="editForm" action="${ctx}/admin/account/save" method="post" cssStyle="padding:10px 20px;">
				<table>
					<tr>
					...
					<tr>
						...
						<td>
							<form:input path="status" id="status_edit" required="true" cssStyle="width:185px;"/>
						</td>
					</tr>
					<tr>
						<td><form:label	for="gender" path="gender" cssClass="easyui-validatebox">性别：</form:label></td>
						<td>
							<form:input path="gender" id="gender_edit" cssClass="easyui-validatebox" editable='false' cssStyle="width:185px;"/>
						</td>
						<td><form:label	for="birthday" path="birthday" cssClass="easyui-validatebox">生日：</form:label></td>
						<td><form:input path="birthday" cssClass="easyui-datebox datebox-f combo-f" cssStyle="width:185px;"/></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td colspan="3"><form:input path="address" cssStyle="width:485px;"/></td>
					</tr>
				</table>
				<form:hidden path="id"/>
				<div style="text-align: center; padding: 5px;">

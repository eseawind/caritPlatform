...
public class AttachmentUtil {
	...
	private AttachmentUtil(String path){
		try {
			...
			logger.debug(path+"/platform_attachment.properties");
			URL url=new URL(path+"/platform_attachment.properties");
			p.load(url.openStream());
			logger.info("init AttachmentUtil INSTANCE end...");
		} catch (MalformedURLException e) {
			logger.error("read file["+path+"/platform_attachment.properties"+"] error...", e);
		} catch (IOException e) {
	...
	public static void main(String[] args) throws IOException {
		init("http://localhost");
		System.out.println(getValue("attachment.video"));

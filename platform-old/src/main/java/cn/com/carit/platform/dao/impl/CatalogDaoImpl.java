...
public class CatalogDaoImpl  extends BaseDaoImpl implements CatalogDao<Catalog> {
	...
	public int add(Catalog t) {
		String sql = "insert into t_catalog(name, description, display_index, status, update_time, create_time) values(?, ?, ?, ?, now(), now())";
		if (log.isDebugEnabled()) {
			log.debug(String.format("\n%1$s\n", sql));
		}
		return jdbcTemplate.update(sql, t.getName(), t.getDescription(),
				t.getDisplayIndex(), t.getStatus());
	}
	...
	public Catalog queryById(int id) {
		String sql = "select * form t_catalog where id=?";
		if (log.isDebugEnabled()) {
			log.debug(String.format("\n%1$s\n", sql));
		}
	...
	public int checkExisted(String name) {
		String sql="select 1 from t_catalog where name=?";
		if (log.isDebugEnabled()) {
			log.debug(String.format("\n%1$s\n", sql));
		}
